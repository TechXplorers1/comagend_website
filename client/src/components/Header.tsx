...
export default function Header() {
  ...
  const handleGetInvolvedClick = () => {
    if (location === "/") {
      ...
      // go to home with hash – browser will scroll to section
  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 bg-white transition-all duration-300 ${
        isScrolled ? "border-b shadow-sm py-3" : "py-4"
      }`}
      initial={{ y: -80 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          {/* Logo with subtle rotation hover */}
            ...
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <motion.a
                  ...
                  transition={{ type: "spring", stiffness: 200 }}
                  className={`relative font-sans font-medium hover:text-primary ${
                    location === link.href
                      ? "text-primary"
                      : "text-foreground/80"
                  }`}
